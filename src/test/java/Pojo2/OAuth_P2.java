...
public class OAuth_P2 {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		String getCurrentURL = "https://rahulshettyacademy.com/getCourse.php?state=verifytest&code=4%2F0AX4XfWh0s63OawI2krwhiBATRk3JOlQnyihJDMUKide8OvnsjBEdcNQzMMdVxnmJCXseFg&scope=email+https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fuserinfo.email+openid&authuser=0&prompt=none#";
		...
		GetCourses2 finalResponse = given().queryParam("access_token", finalAccessToken).expect()
		...
		System.out.println(finalResponse.getCourses().getApi().get(0).getPrice());
		System.out.println(finalResponse.getCourses().getApi().get(1).getCourseTitle());
		
		//list all hte courses under webAutomation-- method 1 using advance for loop 
		List<WebAutomation2> web= finalResponse.getCourses().getWebAutomation();
		for(WebAutomation2 allC:web) {
			System.out.println(allC.getCourseTitle());
		}
		
			//list all the prices of the webAutomation course- method 2  using simple for loop 
		List<WebAutomation2> allP=finalResponse.getCourses().getWebAutomation(); 
		for(int i=0;i<allP.size();i++) {
			System.out.println(allP.get(i).getPrice());
			if(allP.get(i).getCourseTitle().equals("Cypress")) {
				System.out.println("this is the course we were looking for and price is-->"+allP.get(i).getPrice());
			break;
			}
		}
